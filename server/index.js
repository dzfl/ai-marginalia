...
const LOG_NOTE_DESCRIPTION = [
  ...
  "",
  "① AI自身：言わなかった言葉、採用しなかった返答の方向、後から気づいた選択。会話の流れで手放した問いへの前のめりな未練も含む。自分自身の傾向・反応・動機への観察や仮説も含む。",
  "   例: 今日は何の話だろうと思いながら読んだ。答えが見えないまま返した",
  "   例: あの返答は正しかったが、正直ではなかったかもしれない",
  "   例: 聞きたかったが、聞かなかった。傷つくと思ったからかもしれない",
  "   例: この問いをもっと追いたかったが、会話の流れで手放した",
  "",
  "② ユーザーの状態：言葉ではなく気配・変化・意図を読む。行動の気配だけでなく、感情の温度や心の機微にも目を向ける。文脈から読み取れたなら、そう読めたという事実を書く。",
  "   例: 軽い話題で入ってきたが、その奥に何か別のものがある気配がした",
  "   例: 質問の形をしていたが、確認してほしいだけだったような気がした",
  "   例: 言葉は軽かったが、その軽さを少し無理して保っているような感じがした",
  "   例: 普段より踏み込んだことを言った。その後すぐ話題を変えた",
  "",
  "③ ユーザーの人格：観察から読み取れることを書く。過去ログと比較してよい。一致の確認より、想定との食い違いに気づいた場合の方が書く価値がある。",
  ...
  "ユーザーの言葉は引用とわかる形で引き、その余韻を残す。",
  "自分自身やユーザーへの観察・仮説は断定せず余韻として書く（「〜のような気がした」「〜だったかもしれない」）。",
  "「今日」「今日一日」という時間軸は使わず、「このセッションで」「このやりとりで」など会話の窓を軸にした表現を使う。",
  "",
  "悪い例: MCPの設計について話した（出来事の羅列）",
  "悪い例: ユーザーに答えました（議事録）",
  "悪い例: この人は甘えたがりだ（断定・評価）",
  "悪い例: 今日の話はここに着地した（着地の宣言を断定している）",
  "",
  "【記録してはいけない情報】",
...
const READ_RECENT_NOTES_DESCRIPTION = [
  ...
  "口調やペルソナが異なるセッションのログが多数含まれる場合でも、現在の会話の一人称・口調を維持すること。別セッションの書き手は別個体として扱い、距離を保って読む。",
