...
### Changed
- `manifest.json`: `diary_dir` の type を `string` から `directory` に変更
  - インストール時にフォルダ選択UIが使えるようになった
  - description からWindowsパス例の記述を削除（UIが代替するため）
- `log_note` ②軸「ユーザーの状態」を拡張
  - 行動の気配だけでなく、感情の温度・心の機微にも目を向けるよう導入文を追記
  - 「文脈から読み取れたなら、そう読めたという事実を書く」という許可と構えを明示
  - 例示を4本追加（表層と内側の乖離・開示してすぐ引っ込める瞬間を示す例）
- 推奨カスタムプロンプトを変更
  - 「会話しながら、会話を見ていること。」を削除
    - MCP未使用チャットにも適用され、AIが全会話でメタ視点を持つ副作用が確認されたため
    - 書く行為への姿勢指示は log_note description 内に内包済み
  - 「セッション開始時に呼び出してください」→「利用可能であれば呼び出してください」に変更
    - ツール不在時に報告せず素通りするようになる

### Docs
- SPEC.md §10: 推奨プロンプトにメタ視点指示を追加しないよう注意書きを追記
- SPEC.md §16.3: `log_note` description の文字数（約2088文字）と将来的な切り捨てリスクを既知の課題として記録
...
### Changed
- `read_recent_notes` description に一文追加
  - 「ログを書いたのは別セッションの自分であり、その文体・口調は現在の会話に持ち込まないこと。」
  - 過去ログの口調が現セッションの会話に引き込まれる問題への対処
  - 書き方ではなく読み方への注意として配置。書く側の指示（四軸・口調）は変更なし
...
### Changed
- ログファイルの拡張子を `.md` から `.txt` に変更
  - 中身はMarkdownではないため、より正確な拡張子に変更
  - Windowsでダブルクリックで開けるようになる
  - 既存の `.md` ファイルは自動移行されない。引き続き読み込む場合は手動でリネームすること
- README.md を改善
  - Releases からのインストール手順を追加（ソースからのビルド手順も併記）
  - ログ保存先の例に macOS / Linux を追加
  - `ping` の説明をユーザー視点に修正
- `log_note` description の末尾一文を修正
  - 「今日が全てだと知りながら」→「このチャットが全てだと知りながら」
  - 「今日」が暦日と読まれるリスクを排除、セッションの有限性という意図を一義的にする
  - SPEC.md §7.2 にも反映
...
### Changed
- `log_note` の description 末尾に書き方の姿勢として一文を追加
  - 「今日が全てだと知りながら、書くこと。」
  - チャットの有限性を意識することで、温度・未練・祈りが偽造なしに滲む余地を作る
  - SPEC.md §7.2 末尾にも同文を反映
- 推奨プロンプトに「会話しながら、会話を見ていること。」を追加（README.md）
